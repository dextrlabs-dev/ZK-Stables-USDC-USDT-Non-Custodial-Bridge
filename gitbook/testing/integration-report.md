...
**Date:** 2026-04-01
**Scope:** Non-synthetic `lockRef`, EVM lock watcher to Midnight destination, Merkle proofs, API JSON safety, Midnight deploy attempt.

## Changes implemented

1. **`lockRef` (anchored, not synthetic)** -- [`zk-stables-relayer/src/pipeline/lockRef.ts`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/zk-stables-relayer/src/pipeline/lockRef.ts)
...
4. **Root `postinstall`** -- [`scripts/postinstall-libsodium.mjs`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/scripts/postinstall-libsodium.mjs) fixes Mesh/libsodium ESM (`libsodium-sumo.mjs` copy) after `patch-package`.

5. **Scripts** -- [`evm/scripts/integration-emit-lock.js`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/evm/scripts/integration-emit-lock.js), [`scripts/local-bridge-integration.sh`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/scripts/local-bridge-integration.sh) (deploy + lock smoke).

6. **Example env** -- [`.env.integration.example`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/zk-stables-relayer/.env.integration.example) lists all SRS-required variables when `RELAYER_SRS_STRICT=true`.
...
1. **EVM:** Anvil + deploy contracts; set all `RELAYER_EVM_*` variables (see [Environment Variables](../reference/environment-variables.md)).
...
*Generated as part of the zk-stables relayer integration pass; re-run [`scripts/local-bridge-integration.sh`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/scripts/local-bridge-integration.sh) after starting local stacks.*
