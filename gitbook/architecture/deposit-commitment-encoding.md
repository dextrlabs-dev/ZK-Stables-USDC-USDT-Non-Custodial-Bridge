...
Recommended construction using SHA-256 or the curve hash used by your SNARK stack (keep **one** choice across chains):
...
- `||` is concatenation; lengths are fixed so parsers are unambiguous.
...
- [`zk-stables.compact`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/contract/src/zk-stables.compact) stores **`depositCommitment`** as `Bytes<32>` at deploy time; the **DApp must pass** the same bytes produced by this spec after off-chain verification.
...
Source: [`contract/docs/DEPOSIT_COMMITMENT_ENCODING.md`](https://github.com/dextrlabs-dev/ZK-Stables-USDC-USDT-Non-Custodial-Bridge/blob/main/contract/docs/DEPOSIT_COMMITMENT_ENCODING.md)
