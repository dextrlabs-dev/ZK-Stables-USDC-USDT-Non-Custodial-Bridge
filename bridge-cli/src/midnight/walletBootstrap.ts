/**
 * Midnight wallet bootstrap (undeployed / local stack) — aligned with
 * `local-cli/src/wallet.ts` for balance reads in the dashboard.
 */
...
const INDEXER_PORT = Number.parseInt(process.env.BRIDGE_CLI_MIDNIGHT_INDEXER_PORT ?? process.env.INDEXER_PORT ?? '8088', 10);
const NODE_PORT = Number.parseInt(process.env.BRIDGE_CLI_MIDNIGHT_NODE_PORT ?? process.env.NODE_PORT ?? '9944', 10);

function provingServerUrl(): URL {
  const explicit =
    process.env.BRIDGE_CLI_MIDNIGHT_PROOF_SERVER?.replace(/\/$/, '') ??
    process.env.MIDNIGHT_PROOF_SERVER?.replace(/\/$/, '');
  if (explicit) return new URL(explicit);
  const port = Number.parseInt(process.env.BRIDGE_CLI_MIDNIGHT_PROOF_SERVER_PORT ?? process.env.PROOF_SERVER_PORT ?? '6300', 10);
  return new URL(`http://127.0.0.1:${port}`);
}

const INDEXER_HTTP_URL =
  process.env.BRIDGE_CLI_MIDNIGHT_INDEXER_HTTP?.trim() ||
  `http://127.0.0.1:${INDEXER_PORT}/api/v4/graphql`;
const INDEXER_WS_URL =
  process.env.BRIDGE_CLI_MIDNIGHT_INDEXER_WS?.trim() ||
  `ws://127.0.0.1:${INDEXER_PORT}/api/v4/graphql/ws`;

const baseConfiguration: ShieldedConfiguration & DustConfiguration = {
  networkId: (process.env.BRIDGE_CLI_MIDNIGHT_NETWORK_ID?.trim() || 'undeployed') as ShieldedConfiguration['networkId'],
  costParameters: {
...
export async function initWalletWithSeed(seed: Buffer): Promise<WalletContext> {
  ...
  const relayURL = new URL(
    process.env.BRIDGE_CLI_MIDNIGHT_NODE_WS?.trim() || `ws://127.0.0.1:${NODE_PORT}`,
  );
